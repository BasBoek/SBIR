...
ZONAL_STATS <- function(images, input_perceeldata, STATS_dir, minimum_nr_of_pixels){
  ...
  for(IMAGE in 1:length(images)){ # calculate for every raster
    ...
    if(checkmax > 0){
      for(PERCEEL in 1:length(sel_percelen)){
        ...
        if(maxpixels > minimum_nr_of_pixels){ # Berekent het theoretische aantal pixels die binnen een perceel kunnen liggen.
    ...
    write.csv(result,paste(STATS_dir, "perceelstats_", run, ".csv", sep=""), row.names=F)
  }
}
